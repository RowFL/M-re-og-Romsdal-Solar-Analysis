    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                ...
                # This is the crucial message if the Client ID is rejected
...
def get_station_id_robust():
    """
    Tries to retrieve the ID of a sample weather station via generic search.
    If the generic search fails, it uses a well-known, fixed station ID as a fallback 
    to ensure the rest of the script can run.
    """
    print("\n--- 1. FETCHING A SAMPLE STATION ID ---")
    
    # --- Attempt 1: Generic Search for any SensorSystem ---
    ...
    if result and 'data' in result and result['data']:
        ...
        print(f"Successfully retrieved station ID via generic search: {station_id}")
        return station_id
    
    print("Generic station search failed (could be due to no results or authentication failure).")
    
    # --- Attempt 2: Fallback to a well-known, fixed station ID (Blindern, Oslo) ---
    FALLBACK_STATION_ID = "SN90450" 
    print(f"Falling back to known station ID: {FALLBACK_STATION_ID}")
...
if __name__ == '__main__':
    # Use the robust function to get the station ID
    station_id = get_station_id_robust()
